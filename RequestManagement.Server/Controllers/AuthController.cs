...
public class AuthController : AuthService.AuthServiceBase
{
    ...
    public override async Task<AuthorizeResponse> Authorize(AuthorizeRequest request, ServerCallContext context)
    {
        ...
        var isAuthorized = await _authService.AuthorizeAsync(request.UserId, (UserRole)request.RequiredRole);
    ...
    /// </summary>
    private string GenerateJwtToken(RequestManagement.Common.Models.User user)
    {
    ...
    {
        var user = await _authService.RefreshTokenAsync(request.Token, request.IpAddress);
